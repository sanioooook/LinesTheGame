...
export function placeNewBallsOnBoards(
  board: GameBoard,
  balls: Ball[]
): Field[] {
  ...
  for (
    let i = 0;
    i < (freeFields.length <= 3 ? freeFields.length + 1 : balls.length);
    i++
  ) {
    const field = freeFields[Math.floor(Math.random() * freeFields.length)];
    fields.push({...field, ball: balls[i]});
    const index = freeFields.findIndex(
      (x) => x.j === field.j && x.i === field.i
    );
    if (index !== -1) {
...
export function generateNewBalls(count: number): Ball[] {
  ...
  for (let i = 0; i < count; i++) {
    const color = Math.floor(
      (Math.random() * Object.keys(BallColor).length) / 2
    ) as BallColor;
