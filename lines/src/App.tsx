...
import './App.scss';
import {changeLanguage, undoMove} from './store/actions/gameBoard.actions';
...
export const App: React.FC = () => {
  ...
  const canUndo = useAppSelector((state) => (state.gameBoard.history?.length ?? 0) > 0);
    ...
    <div className={`main`}>
      <div className={`navigations`}>
        ...
        <div className={`buttons-and-selector`}>
          ...
          <button className={`button`} onClick={() => setIsLeaderboardOpen(true)}>
            {t('leaderboard')}
          </button>
          <Modal
          ...
          <button className={'button'} disabled={!canUndo || isAnimating} onClick={() => dispatch(undoMove())}>
            {t('undo')}
          </button>
