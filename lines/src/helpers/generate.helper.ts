...
export function placeNewBallsOnBoards(board: GameBoard, balls: Ball[], random = Math.random): Field[] {
  ...
  for (let i = 0; i < (freeFields.length <= 3 ? freeFields.length + 1 : balls.length); i++) {
    const field = freeFields[Math.floor(random() * freeFields.length)];
...
export function generateNewBalls(count: number, random = Math.random): Ball[] {
  const balls: Ball[] = [];

  for (let i = 0; i < count; i++) {
    const color = Math.floor((random() * Object.keys(BallColor).length) / 2) as BallColor;
